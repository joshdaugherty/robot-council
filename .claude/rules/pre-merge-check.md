...
**Confirm the enforcement is real before relying on it.** `gh api repos/joshdaugherty/robot-council/rulesets --jq '.[] | "\(.name) \(.enforcement)"'` must show the ruleset as `active`. While it is not, nothing above is enforced, and confirming a successful `ci-passed` on the current head of a branch that is current with `main` is your job again.
