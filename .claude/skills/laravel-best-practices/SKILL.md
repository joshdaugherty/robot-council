...
This is a package, not an application: `composer.json` admits a range of Laravel releases (`^13.0`), so an API is only safe to use if it exists in the **lowest** release that constraint admits, not just in the version installed for development. A method added in a later 13.x minor needs the constraint raised first.
