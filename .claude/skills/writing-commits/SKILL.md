...
- **`composer refactor` / `vendor/bin/rector`** rewrites every file under `src/`, `config/`, and `database/` that a rule matches, not only the ones you touched. Stage its changes deliberately, or run `composer test:refactor` to see them first.
