...
- **`ThrottleRequests` is in the framework's middleware priority list and the package's middleware is not**, so the router sorts a route's limiter ahead of its principal middleware whatever order the route declares. A limiter keyed on the principal resolves it through the guard rather than reading what the middleware left on the request.
...
- **The change feed is written through `Support\FleetEvents`, never by inserting a row.** It takes a transaction-scoped advisory lock on Postgres before inserting, because Postgres hands out sequence values outside the transaction: two concurrent writers can take IDs 5 and 6 and commit 6 first, and a reader polling in between passes 6 and never sees 5. SQLite serializes writers and cannot show this, so the proof lives in the `cross-connection` group.
...
- **Do not name a method on a queued job `retryAfter` or `backoff`.** Both are framework hooks, and `driftingly/rector-laravel` renames the first to the second — which silently turned a private helper into a method `Illuminate\Queue\Queue` calls itself, with a signature it does not have.
- `database/migrations/` — the package's own tables, loaded by the provider so `php artisan migrate` picks them up. `robot_council_github_identities` maps a host user to a GitHub account, and the package owns it because that mapping is what the access lists are checked against. `robot_council_installations`, `robot_council_agent_sessions`, and `robot_council_device_codes` hold enrollment.
